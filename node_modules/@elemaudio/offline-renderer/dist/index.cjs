var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getOwnPropSymbols = Object.getOwnPropertySymbols;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __propIsEnum = Object.prototype.propertyIsEnumerable;
var __objRest = (source, exclude) => {
  var target = {};
  for (var prop in source)
    if (__hasOwnProp.call(source, prop) && exclude.indexOf(prop) < 0)
      target[prop] = source[prop];
  if (source != null && __getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(source)) {
      if (exclude.indexOf(prop) < 0 && __propIsEnum.call(source, prop))
        target[prop] = source[prop];
    }
  return target;
};
var __commonJS = (cb, mod) => function __require() {
  return mod || (0, cb[__getOwnPropNames(cb)[0]])((mod = { exports: {} }).exports, mod), mod.exports;
};
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toESM = (mod, isNodeMode, target) => (target = mod != null ? __create(__getProtoOf(mod)) : {}, __copyProps(
  // If the importer is in node compatibility mode or this is not an ESM
  // file that has been converted to a CommonJS file using a Babel-
  // compatible transform (i.e. "__esModule" has not been set), then set
  // "default" to the CommonJS "module.exports" for node compatibility.
  isNodeMode || !mod || !mod.__esModule ? __defProp(target, "default", { value: mod, enumerable: true }) : target,
  mod
));
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var __async = (__this, __arguments, generator) => {
  return new Promise((resolve, reject) => {
    var fulfilled = (value) => {
      try {
        step(generator.next(value));
      } catch (e) {
        reject(e);
      }
    };
    var rejected = (value) => {
      try {
        step(generator.throw(value));
      } catch (e) {
        reject(e);
      }
    };
    var step = (x) => x.done ? resolve(x.value) : Promise.resolve(x.value).then(fulfilled, rejected);
    step((generator = generator.apply(__this, __arguments)).next());
  });
};

// elementary-wasm.cjs
var require_elementary_wasm = __commonJS({
  "elementary-wasm.cjs"(exports2, module2) {
    var Module2 = (() => {
      var _scriptDir = typeof document !== "undefined" && document.currentScript ? document.currentScript.src : void 0;
      return function(moduleArg = {}) {
        var _a;
        var l = moduleArg, aa, p;
        l.ready = new Promise((a2, b) => {
          aa = a2;
          p = b;
        });
        if ("function" !== typeof ((_a = globalThis == null ? void 0 : globalThis.crypto) == null ? void 0 : _a.getRandomValues)) {
          let a2 = function(b) {
            for (var c = 0; c < b.length; c++) b[c] = 256 * Math.random() | 0;
          };
          var a = a2;
          "object" === typeof globalThis.crypto ? globalThis.crypto.getRandomValues = a2 : globalThis.crypto = { getRandomValues: a2 };
        }
        var ba = Object.assign({}, l), ca;
        ca = (a2) => {
          if ("function" == typeof readbuffer) return new Uint8Array(readbuffer(a2));
          a2 = read(a2, "binary");
          "object" == typeof a2 || w();
          return a2;
        };
        "undefined" == typeof clearTimeout && (globalThis.clearTimeout = () => {
        });
        "undefined" == typeof setTimeout && (globalThis.setTimeout = (a2) => "function" == typeof a2 ? a2() : w());
        "undefined" != typeof print && ("undefined" == typeof console && (console = {}), console.log = print, console.warn = console.error = "undefined" != typeof printErr ? printErr : print);
        var da = l.printErr || console.error.bind(console);
        Object.assign(l, ba);
        ba = null;
        var ea;
        l.wasmBinary && (ea = l.wasmBinary);
        "object" != typeof WebAssembly && w("no native wasm support detected");
        "undefined" == typeof atob && ("undefined" != typeof global && "undefined" == typeof globalThis && (globalThis = global), globalThis.atob = function(a2) {
          var b = "", c = 0;
          a2 = a2.replace(/[^A-Za-z0-9\+\/=]/g, "");
          do {
            var d = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a2.charAt(c++));
            var e = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a2.charAt(c++));
            var g = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a2.charAt(c++));
            var f = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a2.charAt(c++));
            d = d << 2 | e >> 4;
            e = (e & 15) << 4 | g >> 2;
            var h = (g & 3) << 6 | f;
            b += String.fromCharCode(d);
            64 !== g && (b += String.fromCharCode(e));
            64 !== f && (b += String.fromCharCode(h));
          } while (c < a2.length);
          return b;
        });
        var x, fa = false, ha, y, z, ia, A, B, ja, ka;
        function la() {
          var a2 = x.buffer;
          l.HEAP8 = ha = new Int8Array(a2);
          l.HEAP16 = z = new Int16Array(a2);
          l.HEAPU8 = y = new Uint8Array(a2);
          l.HEAPU16 = ia = new Uint16Array(a2);
          l.HEAP32 = A = new Int32Array(a2);
          l.HEAPU32 = B = new Uint32Array(a2);
          l.HEAPF32 = ja = new Float32Array(a2);
          l.HEAPF64 = ka = new Float64Array(a2);
        }
        var ma = [], na = [], oa = [];
        function pa() {
          var a2 = l.preRun.shift();
          ma.unshift(a2);
        }
        var D = 0, qa = null, E = null;
        function w(a2) {
          var _a2;
          (_a2 = l.onAbort) == null ? void 0 : _a2.call(l, a2);
          a2 = "Aborted(" + a2 + ")";
          da(a2);
          fa = true;
          a2 = new WebAssembly.RuntimeError(a2 + ". Build with -sASSERTIONS for more info.");
          p(a2);
          throw a2;
        }
        var ra = (a2) => a2.startsWith("data:application/octet-stream;base64,"), F;
        F = "data:application/octet-stream;base64,";
        if (!ra(F)) {
          var sa = F;
          F = l.locateFile ? l.locateFile(sa, "") : "" + sa;
        }
        function ta() {
          var a2 = F;
          return Promise.resolve().then(() => {
            if (a2 == F && ea) var b = new Uint8Array(ea);
            else {
              if (ra(a2)) {
                b = atob(a2.slice(37));
                for (var c = new Uint8Array(b.length), d = 0; d < b.length; ++d) c[d] = b.charCodeAt(d);
                b = c;
              } else b = void 0;
              if (!b) if (ca) b = ca(a2);
              else throw "both async and sync fetching of the wasm failed";
            }
            return b;
          });
        }
        function ua(a2, b) {
          return ta().then((c) => WebAssembly.instantiate(c, a2)).then((c) => c).then(b, (c) => {
            da(`failed to asynchronously prepare wasm: ${c}`);
            w(c);
          });
        }
        function va(a2, b) {
          return ua(a2, b);
        }
        var wa = (a2) => {
          for (; 0 < a2.length; ) a2.shift()(l);
        };
        class xa {
          constructor(a2) {
            this.R = a2 - 24;
          }
        }
        var ya = 0, Aa = 0, Ba, G = (a2) => {
          for (var b = ""; y[a2]; ) b += Ba[y[a2++]];
          return b;
        }, H = {}, I = {}, Ca = {}, J, Da = (a2) => {
          throw new J(a2);
        }, K, M = (a2, b, c) => {
          function d(h) {
            h = c(h);
            if (h.length !== a2.length) throw new K("Mismatched type converter count");
            for (var m = 0; m < a2.length; ++m) L(a2[m], h[m]);
          }
          a2.forEach(function(h) {
            Ca[h] = b;
          });
          var e = Array(b.length), g = [], f = 0;
          b.forEach((h, m) => {
            I.hasOwnProperty(h) ? e[m] = I[h] : (g.push(h), H.hasOwnProperty(h) || (H[h] = []), H[h].push(() => {
              e[m] = I[h];
              ++f;
              f === g.length && d(e);
            }));
          });
          0 === g.length && d(e);
        };
        function Ea(a2, b, c = {}) {
          var d = b.name;
          if (!a2) throw new J(`type "${d}" must have a positive integer typeid pointer`);
          if (I.hasOwnProperty(a2)) {
            if (c.sa) return;
            throw new J(`Cannot register type '${d}' twice`);
          }
          I[a2] = b;
          delete Ca[a2];
          H.hasOwnProperty(a2) && (b = H[a2], delete H[a2], b.forEach((e) => e()));
        }
        function L(a2, b, c = {}) {
          if (!("argPackAdvance" in b)) throw new TypeError("registerType registeredInstance requires argPackAdvance");
          return Ea(a2, b, c);
        }
        var Fa = (a2) => {
          throw new J(a2.O.S.P.name + " instance already deleted");
        }, Ga = false, Ha = () => {
        }, Ia = (a2, b, c) => {
          if (b === c) return a2;
          if (void 0 === c.U) return null;
          a2 = Ia(a2, b, c.U);
          return null === a2 ? null : c.pa(a2);
        }, Ja = {}, N = [], Ka = () => {
          for (; N.length; ) {
            var a2 = N.pop();
            a2.O.aa = false;
            a2["delete"]();
          }
        }, O, P = {}, La = (a2, b) => {
          if (void 0 === b) throw new J("ptr should not be undefined");
          for (; a2.U; ) b = a2.da(b), a2 = a2.U;
          return P[b];
        }, Ma = (a2, b) => {
          if (!b.S || !b.R) throw new K("makeClassHandle requires ptr and ptrType");
          if (!!b.V !== !!b.T) throw new K("Both smartPtrType and smartPtr must be specified");
          b.count = { value: 1 };
          return Q(Object.create(a2, { O: { value: b, writable: true } }));
        }, Q = (a2) => {
          if ("undefined" === typeof FinalizationRegistry) return Q = (b) => b, a2;
          Ga = new FinalizationRegistry((b) => {
            b = b.O;
            --b.count.value;
            0 === b.count.value && (b.T ? b.V.Z(b.T) : b.S.P.Z(b.R));
          });
          Q = (b) => {
            var c = b.O;
            c.T && Ga.register(b, { O: c }, b);
            return b;
          };
          Ha = (b) => {
            Ga.unregister(b);
          };
          return Q(a2);
        };
        function Na() {
        }
        var R = (a2, b) => Object.defineProperty(b, "name", { value: a2 }), Oa = (a2, b, c) => {
          if (void 0 === a2[b].Y) {
            var d = a2[b];
            a2[b] = function() {
              if (!a2[b].Y.hasOwnProperty(arguments.length)) throw new J(`Function '${c}' called with an invalid number of arguments (${arguments.length}) - expects one of (${a2[b].Y})!`);
              return a2[b].Y[arguments.length].apply(this, arguments);
            };
            a2[b].Y = [];
            a2[b].Y[d.ea] = d;
          }
        }, Pa = (a2, b) => {
          if (l.hasOwnProperty(a2)) throw new J(`Cannot register public name '${a2}' twice`);
          l[a2] = b;
        }, Qa = (a2) => {
          if (void 0 === a2) return "_unknown";
          a2 = a2.replace(/[^a-zA-Z0-9_]/g, "$");
          var b = a2.charCodeAt(0);
          return 48 <= b && 57 >= b ? `_${a2}` : a2;
        };
        function Ra(a2, b, c, d, e, g, f, h) {
          this.name = a2;
          this.constructor = b;
          this.ba = c;
          this.Z = d;
          this.U = e;
          this.qa = g;
          this.da = f;
          this.pa = h;
          this.va = [];
        }
        var Sa = (a2, b, c) => {
          for (; b !== c; ) {
            if (!b.da) throw new J(`Expected null or instance of ${c.name}, got an instance of ${b.name}`);
            a2 = b.da(a2);
            b = b.U;
          }
          return a2;
        };
        function Ta(a2, b) {
          if (null === b) {
            if (this.ha) throw new J(`null is not a valid ${this.name}`);
            return 0;
          }
          if (!b.O) throw new J(`Cannot pass "${Ua(b)}" as a ${this.name}`);
          if (!b.O.R) throw new J(`Cannot pass deleted object as a pointer of type ${this.name}`);
          return Sa(b.O.R, b.O.S.P, this.P);
        }
        function Va(a2, b) {
          if (null === b) {
            if (this.ha) throw new J(`null is not a valid ${this.name}`);
            if (this.ga) {
              var c = this.wa();
              null !== a2 && a2.push(this.Z, c);
              return c;
            }
            return 0;
          }
          if (!b || !b.O) throw new J(`Cannot pass "${Ua(b)}" as a ${this.name}`);
          if (!b.O.R) throw new J(`Cannot pass deleted object as a pointer of type ${this.name}`);
          if (!this.fa && b.O.S.fa) throw new J(`Cannot convert argument of type ${b.O.V ? b.O.V.name : b.O.S.name} to parameter type ${this.name}`);
          c = Sa(b.O.R, b.O.S.P, this.P);
          if (this.ga) {
            if (void 0 === b.O.T) throw new J("Passing raw pointer to smart pointer is illegal");
            switch (this.ya) {
              case 0:
                if (b.O.V === this) c = b.O.T;
                else throw new J(`Cannot convert argument of type ${b.O.V ? b.O.V.name : b.O.S.name} to parameter type ${this.name}`);
                break;
              case 1:
                c = b.O.T;
                break;
              case 2:
                if (b.O.V === this) c = b.O.T;
                else {
                  var d = b.clone();
                  c = this.xa(c, S(() => d["delete"]()));
                  null !== a2 && a2.push(this.Z, c);
                }
                break;
              default:
                throw new J("Unsupporting sharing policy");
            }
          }
          return c;
        }
        function Wa(a2, b) {
          if (null === b) {
            if (this.ha) throw new J(`null is not a valid ${this.name}`);
            return 0;
          }
          if (!b.O) throw new J(`Cannot pass "${Ua(b)}" as a ${this.name}`);
          if (!b.O.R) throw new J(`Cannot pass deleted object as a pointer of type ${this.name}`);
          if (b.O.S.fa) throw new J(`Cannot convert argument of type ${b.O.S.name} to parameter type ${this.name}`);
          return Sa(b.O.R, b.O.S.P, this.P);
        }
        function Xa(a2) {
          return this.fromWireType(B[a2 >> 2]);
        }
        function Ya(a2, b, c, d, e, g, f, h, m, k, n) {
          this.name = a2;
          this.P = b;
          this.ha = c;
          this.fa = d;
          this.ga = e;
          this.ua = g;
          this.ya = f;
          this.ma = h;
          this.wa = m;
          this.xa = k;
          this.Z = n;
          e || void 0 !== b.U ? this.toWireType = Va : (this.toWireType = d ? Ta : Wa, this.W = null);
        }
        var Za = (a2, b) => {
          if (!l.hasOwnProperty(a2)) throw new K("Replacing nonexistant public symbol");
          l[a2] = b;
          l[a2].ea = void 0;
        }, $a = [], ab, bb = (a2) => {
          var b = $a[a2];
          b || (a2 >= $a.length && ($a.length = a2 + 1), $a[a2] = b = ab.get(a2));
          return b;
        }, cb = (a2, b) => {
          var c = [];
          return function() {
            c.length = 0;
            Object.assign(c, arguments);
            if (a2.includes("j")) {
              var d = l["dynCall_" + a2];
              d = c && c.length ? d.apply(null, [b].concat(c)) : d.call(null, b);
            } else d = bb(b).apply(null, c);
            return d;
          };
        }, T = (a2, b) => {
          a2 = G(a2);
          var c = a2.includes("j") ? cb(a2, b) : bb(b);
          if ("function" != typeof c) throw new J(`unknown function pointer with signature ${a2}: ${b}`);
          return c;
        }, db, fb = (a2) => {
          a2 = eb(a2);
          var b = G(a2);
          U(a2);
          return b;
        }, gb = (a2, b) => {
          function c(g) {
            e[g] || I[g] || (Ca[g] ? Ca[g].forEach(c) : (d.push(g), e[g] = true));
          }
          var d = [], e = {};
          b.forEach(c);
          throw new db(`${a2}: ` + d.map(fb).join([", "]));
        }, hb = (a2, b) => {
          for (var c = [], d = 0; d < a2; d++) c.push(B[b + 4 * d >> 2]);
          return c;
        }, jb = (a2) => {
          for (; a2.length; ) {
            var b = a2.pop();
            a2.pop()(b);
          }
        };
        function kb(a2) {
          for (var b = 1; b < a2.length; ++b) if (null !== a2[b] && void 0 === a2[b].W) return true;
          return false;
        }
        function lb(a2) {
          var b = Function;
          if (!(b instanceof Function)) throw new TypeError(`new_ called with constructor type ${typeof b} which is not a function`);
          var c = R(b.name || "unknownFunctionName", function() {
          });
          c.prototype = b.prototype;
          c = new c();
          a2 = b.apply(c, a2);
          return a2 instanceof Object ? a2 : c;
        }
        function mb(a2, b, c, d, e, g) {
          var f = b.length;
          if (2 > f) throw new J("argTypes array size mismatch! Must at least get return value and 'this' types!");
          var h = null !== b[1] && null !== c, m = kb(b);
          c = "void" !== b[0].name;
          d = [Da, d, e, jb, b[0], b[1]];
          for (e = 0; e < f - 2; ++e) d.push(b[e + 2]);
          if (!m) for (e = h ? 1 : 2; e < b.length; ++e) null !== b[e].W && d.push(b[e].W);
          m = kb(b);
          e = b.length;
          var k = "", n = "";
          for (f = 0; f < e - 2; ++f) k += (0 !== f ? ", " : "") + "arg" + f, n += (0 !== f ? ", " : "") + "arg" + f + "Wired";
          k = `
        return function (${k}) {
        if (arguments.length !== ${e - 2}) {
          throwBindingError('function ${a2} called with ' + arguments.length + ' arguments, expected ${e - 2}');
        }`;
          m && (k += "var destructors = [];\n");
          var r = m ? "destructors" : "null", q = "throwBindingError invoker fn runDestructors retType classParam".split(" ");
          h && (k += "var thisWired = classParam['toWireType'](" + r + ", this);\n");
          for (f = 0; f < e - 2; ++f) k += "var arg" + f + "Wired = argType" + f + "['toWireType'](" + r + ", arg" + f + "); // " + b[f + 2].name + "\n", q.push("argType" + f);
          h && (n = "thisWired" + (0 < n.length ? ", " : "") + n);
          k += (c || g ? "var rv = " : "") + "invoker(fn" + (0 < n.length ? ", " : "") + n + ");\n";
          if (m) k += "runDestructors(destructors);\n";
          else for (f = h ? 1 : 2; f < b.length; ++f) g = 1 === f ? "thisWired" : "arg" + (f - 2) + "Wired", null !== b[f].W && (k += g + "_dtor(" + g + "); // " + b[f].name + "\n", q.push(g + "_dtor"));
          c && (k += "var ret = retType['fromWireType'](rv);\nreturn ret;\n");
          let [t, u] = [q, k + "}\n"];
          t.push(u);
          b = lb(t).apply(null, d);
          return R(a2, b);
        }
        var nb = (a2) => {
          a2 = a2.trim();
          const b = a2.indexOf("(");
          return -1 !== b ? a2.substr(0, b) : a2;
        };
        class ob {
          constructor() {
            this.X = [void 0];
            this.ia = [];
          }
          get(a2) {
            return this.X[a2];
          }
          has(a2) {
            return void 0 !== this.X[a2];
          }
        }
        var V = new ob(), pb = (a2) => {
          a2 >= V.ja && 0 === --V.get(a2).na && (V.X[a2] = void 0, V.ia.push(a2));
        }, Y = (a2) => {
          if (!a2) throw new J("Cannot use deleted val. handle = " + a2);
          return V.get(a2).value;
        }, S = (a2) => {
          switch (a2) {
            case void 0:
              return 1;
            case null:
              return 2;
            case true:
              return 3;
            case false:
              return 4;
            default:
              a2 = { na: 1, value: a2 };
              var b = V.ia.pop() || V.X.length;
              V.X[b] = a2;
              return b;
          }
        };
        function qb(a2) {
          return this.fromWireType(A[a2 >> 2]);
        }
        for (var rb = { name: "emscripten::val", fromWireType: (a2) => {
          var b = Y(a2);
          pb(a2);
          return b;
        }, toWireType: (a2, b) => S(b), argPackAdvance: 8, readValueFromPointer: qb, W: null }, Ua = (a2) => {
          if (null === a2) return "null";
          var b = typeof a2;
          return "object" === b || "array" === b || "function" === b ? a2.toString() : "" + a2;
        }, sb = (a2, b) => {
          switch (b) {
            case 4:
              return function(c) {
                return this.fromWireType(ja[c >> 2]);
              };
            case 8:
              return function(c) {
                return this.fromWireType(ka[c >> 3]);
              };
            default:
              throw new TypeError(`invalid float width (${b}): ${a2}`);
          }
        }, tb = (a2, b, c) => {
          switch (b) {
            case 1:
              return c ? (d) => ha[d >> 0] : (d) => y[d >> 0];
            case 2:
              return c ? (d) => z[d >> 1] : (d) => ia[d >> 1];
            case 4:
              return c ? (d) => A[d >> 2] : (d) => B[d >> 2];
            default:
              throw new TypeError(`invalid integer width (${b}): ${a2}`);
          }
        }, ub = "undefined" != typeof TextDecoder ? new TextDecoder("utf8") : void 0, vb = "undefined" != typeof TextDecoder ? new TextDecoder("utf-16le") : void 0, wb = (a2, b) => {
          var c = a2 >> 1;
          for (var d = c + b / 2; !(c >= d) && ia[c]; ) ++c;
          c <<= 1;
          if (32 < c - a2 && vb) return vb.decode(y.subarray(a2, c));
          c = "";
          for (d = 0; !(d >= b / 2); ++d) {
            var e = z[a2 + 2 * d >> 1];
            if (0 == e) break;
            c += String.fromCharCode(e);
          }
          return c;
        }, xb = (a2, b, c) => {
          c != null ? c : c = 2147483647;
          if (2 > c) return 0;
          c -= 2;
          var d = b;
          c = c < 2 * a2.length ? c / 2 : a2.length;
          for (var e = 0; e < c; ++e) z[b >> 1] = a2.charCodeAt(e), b += 2;
          z[b >> 1] = 0;
          return b - d;
        }, yb = (a2) => 2 * a2.length, zb = (a2, b) => {
          for (var c = 0, d = ""; !(c >= b / 4); ) {
            var e = A[a2 + 4 * c >> 2];
            if (0 == e) break;
            ++c;
            65536 <= e ? (e -= 65536, d += String.fromCharCode(55296 | e >> 10, 56320 | e & 1023)) : d += String.fromCharCode(e);
          }
          return d;
        }, Ab = (a2, b, c) => {
          c != null ? c : c = 2147483647;
          if (4 > c) return 0;
          var d = b;
          c = d + c - 4;
          for (var e = 0; e < a2.length; ++e) {
            var g = a2.charCodeAt(e);
            if (55296 <= g && 57343 >= g) {
              var f = a2.charCodeAt(++e);
              g = 65536 + ((g & 1023) << 10) | f & 1023;
            }
            A[b >> 2] = g;
            b += 4;
            if (b + 4 > c) break;
          }
          A[b >> 2] = 0;
          return b - d;
        }, Bb = (a2) => {
          for (var b = 0, c = 0; c < a2.length; ++c) {
            var d = a2.charCodeAt(c);
            55296 <= d && 57343 >= d && ++c;
            b += 4;
          }
          return b;
        }, Cb = (a2, b) => {
          var c = I[a2];
          if (void 0 === c) throw a2 = b + " has unknown type " + fb(a2), new J(a2);
          return c;
        }, Db = (a2, b, c) => {
          var d = [];
          a2 = a2.toWireType(d, c);
          d.length && (B[b >> 2] = S(d));
          return a2;
        }, Eb = [], Fb = {}, Gb = (a2) => {
          var b = Fb[a2];
          return void 0 === b ? G(a2) : b;
        }, Hb = () => "object" == typeof globalThis ? globalThis : Function("return this")(), Ib = (a2) => {
          var b = Eb.length;
          Eb.push(a2);
          return b;
        }, Jb = (a2, b) => {
          for (var c = Array(a2), d = 0; d < a2; ++d) c[d] = Cb(B[b + 4 * d >> 2], "parameter " + d);
          return c;
        }, Kb = () => {
          if ("object" == typeof crypto && "function" == typeof crypto.getRandomValues) return (a2) => crypto.getRandomValues(a2);
          w("initRandomDevice");
        }, Lb = (a2) => (Lb = Kb())(a2), Mb = Array(256), Nb = 0; 256 > Nb; ++Nb) Mb[Nb] = String.fromCharCode(Nb);
        Ba = Mb;
        J = l.BindingError = class extends Error {
          constructor(a2) {
            super(a2);
            this.name = "BindingError";
          }
        };
        K = l.InternalError = class extends Error {
          constructor(a2) {
            super(a2);
            this.name = "InternalError";
          }
        };
        Object.assign(Na.prototype, {
          isAliasOf: function(a2) {
            if (!(this instanceof Na && a2 instanceof Na)) return false;
            var b = this.O.S.P, c = this.O.R;
            a2.O = a2.O;
            var d = a2.O.S.P;
            for (a2 = a2.O.R; b.U; ) c = b.da(c), b = b.U;
            for (; d.U; ) a2 = d.da(a2), d = d.U;
            return b === d && c === a2;
          },
          clone: function() {
            this.O.R || Fa(this);
            if (this.O.ca) return this.O.count.value += 1, this;
            var a2 = Q, b = Object, c = b.create, d = Object.getPrototypeOf(this), e = this.O;
            a2 = a2(c.call(b, d, { O: { value: { count: e.count, aa: e.aa, ca: e.ca, R: e.R, S: e.S, T: e.T, V: e.V } } }));
            a2.O.count.value += 1;
            a2.O.aa = false;
            return a2;
          },
          ["delete"]() {
            this.O.R || Fa(this);
            if (this.O.aa && !this.O.ca) throw new J("Object already scheduled for deletion");
            Ha(this);
            var a2 = this.O;
            --a2.count.value;
            0 === a2.count.value && (a2.T ? a2.V.Z(a2.T) : a2.S.P.Z(a2.R));
            this.O.ca || (this.O.T = void 0, this.O.R = void 0);
          },
          isDeleted: function() {
            return !this.O.R;
          },
          deleteLater: function() {
            this.O.R || Fa(this);
            if (this.O.aa && !this.O.ca) throw new J("Object already scheduled for deletion");
            N.push(this);
            1 === N.length && O && O(Ka);
            this.O.aa = true;
            return this;
          }
        });
        l.getInheritedInstanceCount = () => Object.keys(P).length;
        l.getLiveInheritedInstances = () => {
          var a2 = [], b;
          for (b in P) P.hasOwnProperty(b) && a2.push(P[b]);
          return a2;
        };
        l.flushPendingDeletes = Ka;
        l.setDelayFunction = (a2) => {
          O = a2;
          N.length && O && O(Ka);
        };
        Object.assign(Ya.prototype, { ra(a2) {
          this.ma && (a2 = this.ma(a2));
          return a2;
        }, la(a2) {
          var _a2;
          (_a2 = this.Z) == null ? void 0 : _a2.call(this, a2);
        }, argPackAdvance: 8, readValueFromPointer: Xa, fromWireType: function(a2) {
          function b() {
            return this.ga ? Ma(this.P.ba, { S: this.ua, R: c, V: this, T: a2 }) : Ma(this.P.ba, { S: this, R: a2 });
          }
          var c = this.ra(a2);
          if (!c) return this.la(a2), null;
          var d = La(this.P, c);
          if (void 0 !== d) {
            if (0 === d.O.count.value) return d.O.R = c, d.O.T = a2, d.clone();
            d = d.clone();
            this.la(a2);
            return d;
          }
          d = this.P.qa(c);
          d = Ja[d];
          if (!d) return b.call(this);
          d = this.fa ? d.oa : d.pointerType;
          var e = Ia(c, this.P, d.P);
          return null === e ? b.call(this) : this.ga ? Ma(d.P.ba, { S: d, R: e, V: this, T: a2 }) : Ma(d.P.ba, { S: d, R: e });
        } });
        db = l.UnboundTypeError = ((a2, b) => {
          var c = R(b, function(d) {
            this.name = b;
            this.message = d;
            d = Error(d).stack;
            void 0 !== d && (this.stack = this.toString() + "\n" + d.replace(/^Error(:[^\n]*)?\n/, ""));
          });
          c.prototype = Object.create(a2.prototype);
          c.prototype.constructor = c;
          c.prototype.toString = function() {
            return void 0 === this.message ? this.name : `${this.name}: ${this.message}`;
          };
          return c;
        })(Error, "UnboundTypeError");
        V.X.push({ value: void 0 }, { value: null }, { value: true }, { value: false });
        Object.assign(V, { ja: V.X.length });
        l.count_emval_handles = () => {
          for (var a2 = 0, b = V.ja; b < V.X.length; ++b) void 0 !== V.X[b] && ++a2;
          return a2;
        };
        var Pb = { l: (a2, b, c) => {
          var d = new xa(a2);
          B[d.R + 16 >> 2] = 0;
          B[d.R + 4 >> 2] = b;
          B[d.R + 8 >> 2] = c;
          ya = a2;
          Aa++;
          throw ya;
        }, x: () => {
        }, C: (a2, b, c, d) => {
          b = G(b);
          L(a2, { name: b, fromWireType: function(e) {
            return !!e;
          }, toWireType: function(e, g) {
            return g ? c : d;
          }, argPackAdvance: 8, readValueFromPointer: function(e) {
            return this.fromWireType(y[e]);
          }, W: null });
        }, H: (a2, b, c, d, e, g, f, h, m, k, n, r, q) => {
          n = G(n);
          g = T(e, g);
          h && (h = T(f, h));
          k && (k = T(m, k));
          q = T(r, q);
          var t = Qa(n);
          Pa(t, function() {
            gb(`Cannot construct ${n} due to unbound types`, [d]);
          });
          M([a2, b, c], d ? [d] : [], function(u) {
            var _a2;
            u = u[0];
            if (d) {
              var v = u.P;
              var W = v.ba;
            } else W = Na.prototype;
            u = R(n, function() {
              if (Object.getPrototypeOf(this) !== za) throw new J("Use 'new' to construct " + n);
              if (void 0 === C.$) throw new J(n + " has no accessible constructor");
              var ib = C.$[arguments.length];
              if (void 0 === ib) throw new J(`Tried to invoke ctor of ${n} with invalid number of parameters (${arguments.length}) - expected (${Object.keys(C.$).toString()}) parameters instead!`);
              return ib.apply(this, arguments);
            });
            var za = Object.create(W, { constructor: { value: u } });
            u.prototype = za;
            var C = new Ra(n, u, za, q, v, g, h, k);
            if (C.U) {
              var X;
              (_a2 = (X = C.U).ka) != null ? _a2 : X.ka = [];
              C.U.ka.push(C);
            }
            v = new Ya(n, C, true, false, false);
            X = new Ya(n + "*", C, false, false, false);
            W = new Ya(n + " const*", C, false, true, false);
            Ja[a2] = { pointerType: X, oa: W };
            Za(t, u);
            return [v, X, W];
          });
        }, G: (a2, b, c, d, e, g) => {
          var f = hb(b, c);
          e = T(d, e);
          M([], [a2], function(h) {
            h = h[0];
            var m = `constructor ${h.name}`;
            void 0 === h.P.$ && (h.P.$ = []);
            if (void 0 !== h.P.$[b - 1]) throw new J(`Cannot register multiple constructors with identical number of parameters (${b - 1}) for class '${h.name}'! Overload resolution is currently only performed using the parameter count, not actual type info!`);
            h.P.$[b - 1] = () => {
              gb(`Cannot construct ${h.name} due to unbound types`, f);
            };
            M([], f, (k) => {
              k.splice(1, 0, null);
              h.P.$[b - 1] = mb(m, k, null, e, g);
              return [];
            });
            return [];
          });
        }, d: (a2, b, c, d, e, g, f, h, m) => {
          var k = hb(c, d);
          b = G(b);
          b = nb(b);
          g = T(e, g);
          M([], [a2], function(n) {
            function r() {
              gb(`Cannot call ${q} due to unbound types`, k);
            }
            n = n[0];
            var q = `${n.name}.${b}`;
            b.startsWith("@@") && (b = Symbol[b.substring(2)]);
            h && n.P.va.push(b);
            var t = n.P.ba, u = t[b];
            void 0 === u || void 0 === u.Y && u.className !== n.name && u.ea === c - 2 ? (r.ea = c - 2, r.className = n.name, t[b] = r) : (Oa(t, b, q), t[b].Y[c - 2] = r);
            M([], k, function(v) {
              v = mb(q, v, n, g, f, m);
              void 0 === t[b].Y ? (v.ea = c - 2, t[b] = v) : t[b].Y[c - 2] = v;
              return [];
            });
            return [];
          });
        }, B: (a2) => L(a2, rb), u: (a2, b, c) => {
          b = G(b);
          L(a2, { name: b, fromWireType: (d) => d, toWireType: (d, e) => e, argPackAdvance: 8, readValueFromPointer: sb(b, c), W: null });
        }, f: (a2, b, c, d, e) => {
          b = G(b);
          -1 === e && (e = 4294967295);
          e = (h) => h;
          if (0 === d) {
            var g = 32 - 8 * c;
            e = (h) => h << g >>> g;
          }
          var f = b.includes("unsigned") ? function(h, m) {
            return m >>> 0;
          } : function(h, m) {
            return m;
          };
          L(a2, {
            name: b,
            fromWireType: e,
            toWireType: f,
            argPackAdvance: 8,
            readValueFromPointer: tb(b, c, 0 !== d),
            W: null
          });
        }, b: (a2, b, c) => {
          function d(g) {
            return new e(ha.buffer, B[g + 4 >> 2], B[g >> 2]);
          }
          var e = [Int8Array, Uint8Array, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array][b];
          c = G(c);
          L(a2, { name: c, fromWireType: d, argPackAdvance: 8, readValueFromPointer: d }, { sa: true });
        }, t: (a2, b) => {
          b = G(b);
          var c = "std::string" === b;
          L(a2, { name: b, fromWireType: function(d) {
            var e = B[d >> 2], g = d + 4;
            if (c) for (var f = g, h = 0; h <= e; ++h) {
              var m = g + h;
              if (h == e || 0 == y[m]) {
                if (f) {
                  var k = f;
                  var n = y, r = k + (m - f);
                  for (f = k; n[f] && !(f >= r); ) ++f;
                  if (16 < f - k && n.buffer && ub) k = ub.decode(n.subarray(k, f));
                  else {
                    for (r = ""; k < f; ) {
                      var q = n[k++];
                      if (q & 128) {
                        var t = n[k++] & 63;
                        if (192 == (q & 224)) r += String.fromCharCode((q & 31) << 6 | t);
                        else {
                          var u = n[k++] & 63;
                          q = 224 == (q & 240) ? (q & 15) << 12 | t << 6 | u : (q & 7) << 18 | t << 12 | u << 6 | n[k++] & 63;
                          65536 > q ? r += String.fromCharCode(q) : (q -= 65536, r += String.fromCharCode(55296 | q >> 10, 56320 | q & 1023));
                        }
                      } else r += String.fromCharCode(q);
                    }
                    k = r;
                  }
                } else k = "";
                if (void 0 === v) var v = k;
                else v += String.fromCharCode(0), v += k;
                f = m + 1;
              }
            }
            else {
              v = Array(e);
              for (h = 0; h < e; ++h) v[h] = String.fromCharCode(y[g + h]);
              v = v.join("");
            }
            U(d);
            return v;
          }, toWireType: function(d, e) {
            e instanceof ArrayBuffer && (e = new Uint8Array(e));
            var g, f = "string" == typeof e;
            if (!(f || e instanceof Uint8Array || e instanceof Uint8ClampedArray || e instanceof Int8Array)) throw new J("Cannot pass non-string to std::string");
            var h;
            if (c && f) for (g = h = 0; g < e.length; ++g) {
              var m = e.charCodeAt(g);
              127 >= m ? h++ : 2047 >= m ? h += 2 : 55296 <= m && 57343 >= m ? (h += 4, ++g) : h += 3;
            }
            else h = e.length;
            g = h;
            h = Ob(4 + g + 1);
            m = h + 4;
            B[h >> 2] = g;
            if (c && f) {
              if (f = m, m = g + 1, g = y, 0 < m) {
                m = f + m - 1;
                for (var k = 0; k < e.length; ++k) {
                  var n = e.charCodeAt(k);
                  if (55296 <= n && 57343 >= n) {
                    var r = e.charCodeAt(++k);
                    n = 65536 + ((n & 1023) << 10) | r & 1023;
                  }
                  if (127 >= n) {
                    if (f >= m) break;
                    g[f++] = n;
                  } else {
                    if (2047 >= n) {
                      if (f + 1 >= m) break;
                      g[f++] = 192 | n >> 6;
                    } else {
                      if (65535 >= n) {
                        if (f + 2 >= m) break;
                        g[f++] = 224 | n >> 12;
                      } else {
                        if (f + 3 >= m) break;
                        g[f++] = 240 | n >> 18;
                        g[f++] = 128 | n >> 12 & 63;
                      }
                      g[f++] = 128 | n >> 6 & 63;
                    }
                    g[f++] = 128 | n & 63;
                  }
                }
                g[f] = 0;
              }
            } else if (f) for (f = 0; f < g; ++f) {
              k = e.charCodeAt(f);
              if (255 < k) throw U(m), new J("String has UTF-16 code units that do not fit in 8 bits");
              y[m + f] = k;
            }
            else for (f = 0; f < g; ++f) y[m + f] = e[f];
            null !== d && d.push(U, h);
            return h;
          }, argPackAdvance: 8, readValueFromPointer: Xa, W(d) {
            U(d);
          } });
        }, p: (a2, b, c) => {
          c = G(c);
          if (2 === b) {
            var d = wb;
            var e = xb;
            var g = yb;
            var f = () => ia;
            var h = 1;
          } else 4 === b && (d = zb, e = Ab, g = Bb, f = () => B, h = 2);
          L(a2, { name: c, fromWireType: (m) => {
            for (var k = B[m >> 2], n = f(), r, q = m + 4, t = 0; t <= k; ++t) {
              var u = m + 4 + t * b;
              if (t == k || 0 == n[u >> h]) q = d(q, u - q), void 0 === r ? r = q : (r += String.fromCharCode(0), r += q), q = u + b;
            }
            U(m);
            return r;
          }, toWireType: (m, k) => {
            if ("string" != typeof k) throw new J(`Cannot pass non-string to C++ string type ${c}`);
            var n = g(k), r = Ob(4 + n + b);
            B[r >> 2] = n >> h;
            e(k, r + 4, n + b);
            null !== m && m.push(U, r);
            return r;
          }, argPackAdvance: 8, readValueFromPointer: qb, W(m) {
            U(m);
          } });
        }, D: (a2, b) => {
          b = G(b);
          L(a2, { ta: true, name: b, argPackAdvance: 0, fromWireType: () => {
          }, toWireType: () => {
          } });
        }, j: (a2, b, c) => {
          a2 = Y(a2);
          b = Cb(b, "emval::as");
          return Db(b, c, a2);
        }, w: (a2, b, c, d) => {
          a2 = Eb[a2];
          b = Y(b);
          return a2(null, b, c, d);
        }, q: (a2, b, c, d, e) => {
          a2 = Eb[a2];
          b = Y(b);
          c = Gb(c);
          return a2(b, b[c], d, e);
        }, a: pb, i: (a2) => {
          if (0 === a2) return S(Hb());
          a2 = Gb(a2);
          return S(Hb()[a2]);
        }, m: (a2, b, c) => {
          b = Jb(a2, b);
          var d = b.shift();
          a2--;
          var e = "return function (obj, func, destructorsRef, args) {\n", g = 0, f = [];
          0 === c && f.push("obj");
          for (var h = ["retType"], m = [d], k = 0; k < a2; ++k) f.push("arg" + k), h.push("argType" + k), m.push(b[k]), e += `  var arg${k} = argType${k}.readValueFromPointer(args${g ? "+" + g : ""});
`, g += b[k].argPackAdvance;
          e += `  var rv = ${1 === c ? "new func" : "func.call"}(${f.join(", ")});
`;
          d.ta || (h.push("emval_returnValue"), m.push(Db), e += "  return emval_returnValue(retType, destructorsRef, rv);\n");
          h.push(e + "};\n");
          a2 = lb(h).apply(null, m);
          c = `methodCaller<(${b.map((n) => n.name).join(", ")}) => ${d.name}>`;
          return Ib(R(c, a2));
        }, h: (a2, b) => {
          a2 = Y(a2);
          b = Y(b);
          return S(a2[b]);
        }, g: (a2) => {
          4 < a2 && (V.get(a2).na += 1);
        }, n: (a2, b) => {
          a2 = Y(a2);
          b = Y(b);
          return a2 instanceof b;
        }, F: (a2) => {
          a2 = Y(a2);
          return "number" == typeof a2;
        }, E: (a2) => {
          a2 = Y(a2);
          return "string" == typeof a2;
        }, s: () => S([]), r: (a2) => S(Gb(a2)), v: () => S({}), e: (a2) => {
          var b = Y(a2);
          jb(b);
          pb(a2);
        }, k: (a2, b, c) => {
          a2 = Y(a2);
          b = Y(b);
          c = Y(c);
          a2[b] = c;
        }, c: (a2, b) => {
          a2 = Cb(a2, "_emval_take_value");
          a2 = a2.readValueFromPointer(b);
          return S(a2);
        }, o: () => {
          w("");
        }, A: (a2, b, c) => y.copyWithin(a2, b, b + c), z: (a2) => {
          var b = y.length;
          a2 >>>= 0;
          if (2147483648 < a2) return false;
          for (var c = 1; 4 >= c; c *= 2) {
            var d = b * (1 + 0.2 / c);
            d = Math.min(d, a2 + 100663296);
            var e = Math;
            d = Math.max(a2, d);
            a: {
              e = (e.min.call(e, 2147483648, d + (65536 - d % 65536) % 65536) - x.buffer.byteLength + 65535) / 65536;
              try {
                x.grow(e);
                la();
                var g = 1;
                break a;
              } catch (f) {
              }
              g = void 0;
            }
            if (g) return true;
          }
          return false;
        }, y: (a2, b) => {
          Lb(y.subarray(a2, a2 + b));
          return 0;
        } }, Z = function() {
          var _a2;
          function a2(c) {
            var _a3;
            Z = c.exports;
            x = Z.I;
            la();
            ab = Z.L;
            na.unshift(Z.J);
            D--;
            (_a3 = l.monitorRunDependencies) == null ? void 0 : _a3.call(l, D);
            0 == D && (null !== qa && (clearInterval(qa), qa = null), E && (c = E, E = null, c()));
            return Z;
          }
          var b = { a: Pb };
          D++;
          (_a2 = l.monitorRunDependencies) == null ? void 0 : _a2.call(l, D);
          if (l.instantiateWasm) try {
            return l.instantiateWasm(b, a2);
          } catch (c) {
            da(`Module.instantiateWasm callback failed with error: ${c}`), p(c);
          }
          va(b, function(c) {
            a2(c.instance);
          }).catch(p);
          return {};
        }(), Ob = (a2) => (Ob = Z.K)(a2), eb = (a2) => (eb = Z.M)(a2), U = (a2) => (U = Z.N)(a2), Qb;
        E = function Rb() {
          Qb || Sb();
          Qb || (E = Rb);
        };
        function Sb() {
          function a2() {
            if (!Qb && (Qb = true, l.calledRun = true, !fa)) {
              wa(na);
              aa(l);
              if (l.onRuntimeInitialized) l.onRuntimeInitialized();
              if (l.postRun) for ("function" == typeof l.postRun && (l.postRun = [l.postRun]); l.postRun.length; ) {
                var b = l.postRun.shift();
                oa.unshift(b);
              }
              wa(oa);
            }
          }
          if (!(0 < D)) {
            if (l.preRun) for ("function" == typeof l.preRun && (l.preRun = [l.preRun]); l.preRun.length; ) pa();
            wa(ma);
            0 < D || (l.setStatus ? (l.setStatus("Running..."), setTimeout(function() {
              setTimeout(function() {
                l.setStatus("");
              }, 1);
              a2();
            }, 1)) : a2());
          }
        }
        if (l.preInit) for ("function" == typeof l.preInit && (l.preInit = [l.preInit]); 0 < l.preInit.length; ) l.preInit.pop()();
        Sb();
        return moduleArg.ready;
      };
    })();
    if (typeof exports2 === "object" && typeof module2 === "object")
      module2.exports = Module2;
    else if (typeof define === "function" && define["amd"])
      define([], () => Module2);
  }
});

// index.ts
var index_exports = {};
__export(index_exports, {
  default: () => OfflineRenderer
});
module.exports = __toCommonJS(index_exports);
var import_invariant = __toESM(require("invariant"), 1);
var import_core = require("@elemaudio/core");
var import_elementary_wasm = __toESM(require_elementary_wasm(), 1);
var OfflineRenderer = class extends import_core.EventEmitter {
  initialize(options) {
    return __async(this, null, function* () {
      const config = Object.assign({
        numInputChannels: 0,
        numOutputChannels: 2,
        sampleRate: 44100,
        blockSize: 512,
        virtualFileSystem: {}
      }, options);
      const {
        numInputChannels,
        numOutputChannels,
        sampleRate,
        blockSize,
        virtualFileSystem
      } = config;
      this._numInputChannels = numInputChannels;
      this._numOutputChannels = numOutputChannels;
      this._blockSize = blockSize;
      try {
        this._module = yield (0, import_elementary_wasm.default)();
        this._native = new this._module.ElementaryAudioProcessor(numInputChannels, numOutputChannels);
        this._native.prepare(sampleRate, blockSize);
      } catch (e) {
        if (e instanceof WebAssembly.RuntimeError) {
          throw new Error("Failed to load the Elementary WASM backend. Running Elementary within Node.js requires Node v18, or Node v16 with --experimental-wasm-eh enabled.");
        }
        throw e;
      }
      const validVFS = typeof virtualFileSystem === "object" && virtualFileSystem !== null && Object.keys(virtualFileSystem).length > 0;
      if (validVFS) {
        for (let [key, val] of Object.entries(virtualFileSystem)) {
          let result = this._native.addSharedResource(key, val);
          if (!result.success) {
            this.emit("error", new Error(result.message));
          }
        }
      }
      this._renderer = new import_core.Renderer((batch) => {
        return this._native.postMessageBatch(batch);
      });
    });
  }
  render(...args) {
    return __async(this, null, function* () {
      const _a = yield this._renderer.render(...args), { result } = _a, stats = __objRest(_a, ["result"]);
      if (!result.success) {
        return Promise.reject(result);
      }
      return Promise.resolve(stats);
    });
  }
  createRef(kind, props, children) {
    return this._renderer.createRef(kind, props, children);
  }
  process(inputs, outputs) {
    if (!Array.isArray(inputs) || inputs.length !== this._numInputChannels)
      throw new Error(`Invalid input data; expected an array of ${this._numInputChannels} Float32Array buffers.`);
    if (!Array.isArray(outputs) || outputs.length !== this._numOutputChannels)
      throw new Error(`Invalid output data; expected an array of ${this._numOutputChannels} Float32Array buffers.`);
    if (outputs.length === 0) {
      return;
    }
    for (let k = 0; k < outputs[0].length; k += this._blockSize) {
      inputs.forEach((buf, i) => {
        const internalData = this._native.getInputBufferData(i);
        for (let j = 0; j < this._blockSize; ++j) {
          internalData[j] = k + j < buf.length ? buf[k + j] : 0;
        }
      });
      this._native.process(this._blockSize);
      this._native.processQueuedEvents((evtBatch) => {
        evtBatch.forEach(({ type, event }) => {
          this.emit(type, event);
        });
      });
      outputs.forEach((buf, i) => {
        const internalData = this._native.getOutputBufferData(i);
        for (let j = 0; j < this._blockSize; ++j) {
          if (k + j < buf.length) {
            buf[k + j] = internalData[j];
          }
        }
      });
    }
  }
  updateVirtualFileSystem(vfs) {
    const valid = typeof vfs === "object" && vfs !== null;
    (0, import_invariant.default)(valid, "Virtual file system must be an object mapping string type keys to Array<Float32Array> | Float32Array type values");
    Object.keys(vfs).forEach(function(key) {
      const validValue = typeof vfs[key] === "object" && (Array.isArray(vfs[key]) || vfs[key] instanceof Float32Array);
      (0, import_invariant.default)(validValue, "Virtual file system must be an object mapping string type keys to Array<Float32Array> | Float32Array type values");
    });
    for (let [key, val] of Object.entries(vfs)) {
      let result = this._native.addSharedResource(key, val);
      if (!result.success) {
        return result;
      }
    }
    return {
      success: true,
      message: "Ok"
    };
  }
  pruneVirtualFileSystem() {
    this._native.pruneSharedResources();
  }
  listVirtualFileSystem() {
    return this._native.listSharedResources();
  }
  reset() {
    this._native.reset();
  }
  gc() {
    let pruned = this._native.gc();
    this._renderer.prune(pruned);
    return pruned;
  }
  setCurrentTime(t) {
    this._native.setCurrentTime(t);
  }
  setCurrentTimeMs(t) {
    this._native.setCurrentTimeMs(t);
  }
};
